...
def clean_anomaly1(df, index_name="15分钟段", anomaly_name="实际功率"):
    df_summary = df.groupby(index_name, as_index=False)[anomaly_name].agg({"50%": "median", "std": "std"})
    df_summary["up"] = df_summary["50%"] + 3 * (df_summary["std"])
    df_summary["low"] = df_summary["50%"] - 3 * (df_summary["std"])
    df_clean = df.merge(df_summary[[index_name, "up", "low"]], left_on=index_name, right_on=index_name)
    df_clean = df_clean[df_clean[anomaly_name] <= df_clean["up"]]
    df_clean = df_clean[df_clean[anomaly_name] >= df_clean["low"]]
    return df_clean.drop(["low", "up"], axis=1).reset_index(drop=True)


def clean_anomaly2(df, index_name="15分钟段", var_name="实际功率", anomaly_limit=1.5, base_p=0.3):
    df_clean = []
...
def get_data(num, p):
    ...
    train["choose"] = (train["实际功率"] - np.min(train["实际功率"])) / \
                      (np.max(train["实际功率"]) - np.min(train["实际功率"])) * 2 - 1
    train.choose = np.abs(train.choose - train["辐照度"])
    plt.figure(figsize=[64, 8])
    sns.boxplot(x="15分钟段", y="实际功率", data=train)
    plt.savefig("./plot/清洗前/实际功率_%02d.png" % num)
    plt.close()
    train = train[train.choose < 0.75]
    train = train.drop("choose", axis=1)
    train = train.sort_values(by="15分钟段").reset_index(drop=True)
    if num in [3, 6, 7, 9, 10]:
        train = clean_anomaly1(train)
        train = clean_anomaly2(train, index_name="15分钟段", var_name="实际功率", anomaly_limit=1.0, base_p=p * 0.03)
    plt.figure(figsize=[64, 8])
    sns.boxplot(x="15分钟段", y="实际功率", data=train)
    plt.savefig("./plot/清洗后/实际功率_%02d.png" % num)
    plt.close()
    train["id"] = 0
    test = pd.read_csv("./data//test/test_%s.csv" % num, dtype={"id": np.int})
    ...
    data["hour"] = data["时间"].dt.time
    period = data.groupby("hour", as_index=False)["实际功率"].agg({"base_mean_hour": "mean", "base_std_hour": "std"}).reset_index()
    period["hour_num"] = np.abs(period["index"] - period["base_mean_hour"].values.argmax())
    period = period.fillna(0)
    data = data.merge(period[["hour", "hour_num", "base_mean_hour", "base_std_hour"]], left_on="hour", right_on="hour")
    period = data.groupby("15分钟段", as_index=False)["实际功率"].agg({"base_mean_15": "mean", "base_std_15": "std"}).reset_index()
    period["15分钟段_num"] = np.abs(period["index"] - period["base_mean_15"].values.argmax())
    period = period.fillna(0)
    data = data.merge(period[["15分钟段", "15分钟段_num", "base_mean_15", "base_std_15"]], left_on="15分钟段", right_on="15分钟段")
    return data


def plot_scatter(df, fig_num, p):
    y_true = df["True"]
    y_predict = df["Predict"]
    plt.figure()
    plt.scatter(y_true, y_predict, s=2000 / len(y_predict), c="skyblue", label="True vs Predict")
    plt.plot([np.min(y_true), np.max(y_true)], [np.min(y_true), np.max(y_true)], c="r",
             label="Reference line")
    plt.xlabel('True value')
    plt.ylabel('Predict value')
    r = np.corrcoef(y_true, y_predict)
    mae = mean_absolute_error(y_true, y_predict) / p
    plt.title(r'MAE=%.3f $R^2$=%.3f' % (mae, r[0, 1] ** 2))
    plt.legend()
    plt.show()
    plt.savefig("./plot/%02d拟合比较.png" % fig_num)
    plt.close()
    return None
